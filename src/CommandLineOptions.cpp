...
void CommandLineOptions::addBoolOption(string lname, string sname, bool value) {
	string dlname;
	dlname += "--";
	dlname += lname;
...
void CommandLineOptions::addNaturalOption(string lname, string sname,
		int value) {
	string dlname;
	dlname += "--";
	dlname += lname;
...
void CommandLineOptions::addRealOption(string lname, string sname,
		double value) {
	string dlname;
	dlname += "--";
	dlname += lname;
...
void CommandLineOptions::addStringOption(string lname, string sname,
		string value) {
	string dlname;
	dlname += "--";
	dlname += lname;
...
void CommandLineOptions::readOptions(int argc, const char **argv) {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (argument.substr(0, 1) == "-") {
			...
			if (riter_ != realOptions_.end()) {
				if (i < argc - 1)
					realOptions_[key] = (double) atof(
							(arguments_[i + 1]).c_str());
				i++;
				notFound = false;
			}
			...
			if (notFound)
				cout << "Option " << key << " was not found. It is ignored "
						<< endl;
		} else
...
bool CommandLineOptions::getBoolOption(string lname) {
	return boolOptions_["--" + lname];
}

int CommandLineOptions::getNaturalOption(string lname) {
	return naturalOptions_["--" + lname];
}

double CommandLineOptions::getRealOption(string lname) {
	return realOptions_["--" + lname];
}

string CommandLineOptions::getStringOption(string lname) {
	return stringOptions_["--" + lname];
}

vector<string> CommandLineOptions::getRemaining() {
	return remaining_;
}
